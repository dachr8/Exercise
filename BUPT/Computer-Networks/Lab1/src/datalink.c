...
int main(int argc, char** argv) {
	...
	while (true) {
		...
		case FRAME_RECEIVED:                                          // a data or control fraem has arrived
			...
			if (len < 5 || crc32((unsigned char*)& f, len)) {
			...
			case FRAME_DATA:                                          // an undamaged frame has arrived
				...
				if (between(frame_expected, f.seq, too_far) && !arrived[f.seq % NR_BUFS]) {
					...
					arrived[f.seq % NR_BUFS] = true;                  // mark out_buf as full
					memcpy(in_buf[f.seq % NR_BUFS], f.data, len - 7); // insert data into out_buf
		...
		case DATA_TIMEOUT:
			...
			send_frame_k(FRAME_DATA, ack_expected);                   // time out
			break;
		case ACK_TIMEOUT:
